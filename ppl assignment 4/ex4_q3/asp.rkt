...
(define derived?
  (lambda (exp)
    (or (cond? exp) (function-definition? exp) (let? exp) (letrec? exp) (and? exp))))
...
(define and->if
  (lambda (exp)
    (letrec ((expand-predicates
             (lambda (predicates)
               (cond [(empty? predicates) (and-res exp)]
                     [(make-if (car predicates) (expand-predicates (cdr predicates)) #f)]))))
      (expand-predicates (and-predicates exp)))))
